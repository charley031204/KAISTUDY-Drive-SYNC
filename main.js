...
var OAUTH_CLIENT_ID = "234880468147-mjls25oe8g86hjmmg9eoum9s028imlbi.apps.googleusercontent.com";
var OAUTH_CLIENT_SECRET = "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";
var DESKTOP_REDIRECT_URI = "http://127.0.0.1:42813/callback";
var BRIDGE_SERVER_REDIRECT_URI = "https://kaistudy-sync-bridge.vercel.app/";
...
var GoogleDriveSyncPlugin = class extends import_obsidian.Plugin {
  constructor() {
    ...
    this.mobileAuthUrl = null;
    this.localServer = null;
  }
  async onload() {
    await this.loadSettings();
    this.settingTab = new GoogleDriveSyncSettingTab(this.app, this);
    this.addSettingTab(this.settingTab);
    this.addRibbonIcon("sync", "Sync with Google Drive", () => this.syncVault());
    this.toggleAutoSync();
    this.registerObsidianProtocolHandler("kaistudy-sync", async (params) => {
      ...
      if (code) {
        new import_obsidian.Notice("Authorization successful, completing login...");
        await this.exchangeCodeForToken(code, this.authCodeVerifier);
      } else {
        new import_obsidian.Notice("Authentication failed: No code received.");
        this.isAuthenticating = false;
      }
    });
  }
  ...
  // --- AUTHENTICATION SECTION (STABLE & MERGED) ---
  toBase64Url(data) {
    return btoa(String.fromCharCode.apply(null, new Uint8Array(data))).replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
  }
  async generatePKCE() {
    const verifierBytes = crypto.getRandomValues(new Uint8Array(32));
    const verifier = this.toBase64Url(verifierBytes.buffer);
    const challengeBuffer = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(verifier));
    const challenge = this.toBase64Url(challengeBuffer);
    return { verifier, challenge };
  }
  async prepareMobileAuth() {
    if (this.isAuthenticating || this.mobileAuthUrl)
      return;
    this.authState = this.toBase64Url(crypto.getRandomValues(new Uint8Array(16)).buffer);
    ...
    authUrl.searchParams.set("redirect_uri", BRIDGE_SERVER_REDIRECT_URI);
    this.mobileAuthUrl = authUrl.toString();
  }
  async login() {
    if (this.isAuthenticating) {
      new import_obsidian.Notice("Authentication already in progress.");
      return;
    }
    ...
    if (import_obsidian.Platform.isDesktop) {
      try {
        ...
        authUrl.searchParams.set("access_type", "offline");
        authUrl.searchParams.set("prompt", "consent");
        authUrl.searchParams.set("redirect_uri", DESKTOP_REDIRECT_URI);
        if (this.localServer)
          this.localServer.close();
        this.localServer = http.createServer(async (req, res) => {
          ...
          (_a = this.localServer) == null ? void 0 : _a.close();
          this.localServer = null;
          res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
          res.end("<h1>Success!</h1><p>Please return to Obsidian.</p>");
          const code = url.searchParams.get("code");
          if (url.searchParams.get("state") !== this.authState) {
            this.isAuthenticating = false;
            return new import_obsidian.Notice("Authentication failed: state mismatch.");
          }
          if (code) {
            await this.exchangeCodeForToken(code, this.authCodeVerifier);
          } else {
        ...
        }).listen(42813, "127.0.0.1", () => window.open(authUrl.toString()));
      } catch (e) {
        this.isAuthenticating = false;
        new import_obsidian.Notice("Failed to start desktop authentication.");
        console.error(e);
      }
    } else {
      if (this.mobileAuthUrl) {
        open(this.mobileAuthUrl);
      } else {
        this.isAuthenticating = false;
        new import_obsidian.Notice("Authentication not ready. Please close and reopen settings.", 5e3);
      }
    }
  }
  async exchangeCodeForToken(code, codeVerifier) {
    var _a;
    if (!codeVerifier) {
      new import_obsidian.Notice("CRITICAL ERROR: Code Verifier is missing.", 1e4);
      this.isAuthenticating = false;
      return;
    }
    try {
      const payload = {
        ...
        redirect_uri: import_obsidian.Platform.isDesktop ? DESKTOP_REDIRECT_URI : BRIDGE_SERVER_REDIRECT_URI
      };
      if (import_obsidian.Platform.isDesktop) {
      ...
      if (response.status !== 200) {
        const errorDescription = tokens.error_description || tokens.error || "Unknown error";
        new import_obsidian.Notice(`Google API Error: ${errorDescription}`, 15e3);
        throw new Error(`Google API Error: ${errorDescription}`);
      }
      if (tokens.refresh_token) {
        ...
        new import_obsidian.Notice("Critical: Refresh token not received from Google.", 1e4);
        throw new Error("Critical: Refresh token not received.");
      }
    } catch (error) {
  ...
  async getAccessToken() {
    ...
    try {
      const payload = {
        ...
        // Always needed for token refresh
      ...
      if (error.status === 400 || error.status === 401) {
        new import_obsidian.Notice("Connection lost. Please log in again.");
        this.logout();
      }
      return null;
    }
  ...
  async syncVault(isAutoSync = false) {
    ...
    try {
      ...
      statusNotice.setMessage("1/5: Reading states & cleaning remote drive...");
  ...
  async loadRemoteState(accessToken, vaultFolderId) {
    ...
    for (const [path, files] of filesByPath.entries()) {
      let fileToKeep;
      if (files.length > 1) {
        console.warn(`Google Drive Sync: Duplicate files found at "${path}". Cleaning up...`);
        files.sort((a, b) => new Date(b.modifiedTime).getTime() - new Date(a.modifiedTime).getTime());
        fileToKeep = files.shift();
        for (const duplicateFile of files) {
          console.warn(`Google Drive Sync: Deleting older duplicate: ${duplicateFile.name} (ID: ${duplicateFile.id})`);
          await this.deleteRemoteItem(accessToken, duplicateFile.id).catch((err) => {
            console.error(`Failed to delete duplicate file ${duplicateFile.id}`, err);
          });
        }
      } else {
        fileToKeep = files[0];
      }
      const parentId = (_b = fileToKeep.parents) == null ? void 0 : _b[0];
      if (!parentId)
        continue;
      remoteState.set(path, {
        gdriveId: fileToKeep.id,
        name: fileToKeep.name,
        path,
        mtime: new Date(fileToKeep.modifiedTime).getTime(),
        parentId,
        isFolder: fileToKeep.mimeType === "application/vnd.google-apps.folder"
      });
    }
    return remoteState;
  }
  ...
  planFirstSyncMerge(localState, remoteState) {
    ...
    for (const [path, remoteItem] of remoteState.entries()) {
      if (!processedRemotePaths.has(path)) {
        ...
        masterLog.items[syncId] = logEntry;
        if (remoteItem.isFolder) {
          actions.push({ type: "CREATE_LOCAL_FOLDER", remote: remoteItem, log: logEntry });
        } else {
          actions.push({ type: "DOWNLOAD_NEW_FILE", remote: remoteItem, log: logEntry });
        }
      }
    }
  ...
  planActions(localState, remoteState, masterLog, localShadow) {
    ...
    for (const [syncId, mItem] of Object.entries(masterLog.items)) {
      ...
      if (mItem.path !== sItem.path && !actions.some((a) => (a.type === "RENAME_REMOTE" || a.type === "MOVE_REMOTE") && a.log.syncId === syncId)) {
        ...
        if (isRename)
          actions.push({ type: "RENAME_LOCAL", remote: rItem, newLog: mItem, oldLog: sItem });
        else
          actions.push({ type: "MOVE_LOCAL", remote: rItem, newLog: mItem, oldLog: sItem });
      }
      ...
      try {
        switch (action.type) {
          ...
          case "UPDATE_LOCAL_FILE":
            ...
            if (action.log) {
              const downloadedFile = this.app.vault.getAbstractFileByPath(action.remote.path);
              if (downloadedFile instanceof import_obsidian.TFile) {
                const content = await this.app.vault.readBinary(downloadedFile);
                const sha256 = await calculateSha256(content);
                newLog.items[action.log.syncId] = { ...action.log, sha256 };
              } else {
                newLog.items[action.log.syncId] = action.log;
              }
            }
          ...
          case "RENAME_LOCAL":
          case "MOVE_LOCAL":
            const itemToMove = this.app.vault.getAbstractFileByPath(action.oldLog.path);
            if (itemToMove) {
              await this.app.vault.rename(itemToMove, action.newLog.path);
            }
            newLog.items[action.newLog.syncId].path = action.newLog.path;
            break;
          case "DELETE_LOCAL":
...
--${boundary}--`
  ...
  async getOrCreateFolder(accessToken, name, parentId) {
    ...
    const searchUrl = new URL(DRIVE_API_URL + "/files?q=" + encodeURIComponent(query) + "&fields=files(id)");
    const searchResponse = await (0, import_obsidian.requestUrl)({ method: "GET", url: searchUrl.toString(), headers: { "Authorization": `Bearer ${accessToken}` } });
...
var GoogleDriveSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  async display() {
    ...
    if (this.plugin.settings.refreshToken) {
      new import_obsidian.Setting(containerEl).setName("Connected Account").setDesc(`Connected as: ${this.plugin.settings.userEmail || "..."}`).addButton((button) => button.setButtonText("Logout").setWarning().onClick(async () => {
      ...
      if (!import_obsidian.Platform.isDesktop) {
        this.plugin.prepareMobileAuth();
      }
      new import_obsidian.Setting(containerEl).setName("Connect to Google Drive").setDesc("Authorize this plugin to sync your vault.").addButton((button) => button.setButtonText("Connect").setCta().onClick(() => {
        this.plugin.login();
      }));
    }
  }
