...
--${boundary}--`
  ...
  // --- START: AUTHENTICATION REFACTOR (BASED ON EXPERT REVIEW) ---
  ...
  async generatePKCE() {
    ...
    const verifier = this.toBase64Url(verifierBytes);
    const enc = new TextEncoder();
    const digest = await crypto.subtle.digest("SHA-256", enc.encode(verifier));
    const challenge = this.toBase64Url(new Uint8Array(digest));
    return { verifier, challenge };
  }
  // --- Auth Window opener (Crucial for iOS) ------------------------
  openBlankPopupSync() {
    try {
  ...
  openUrlWithFallback(url, preOpened) {
  ...
  async login() {
    ...
    let popup = null;
    try {
      popup = this.openBlankPopupSync();
      this.authState = this.toBase64Url(crypto.getRandomValues(new Uint8Array(16)));
      ...
      console.log("Auth URL (will open):", finalUrl);
      this.openUrlWithFallback(finalUrl, popup);
      ...
      new import_obsidian.Notice("Could not start login. See console for details.");
      try {
        if (popup && !popup.closed)
          popup.close();
      } catch (e) {
