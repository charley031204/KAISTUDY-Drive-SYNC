...
var OAUTH_CLIENT_ID_DESKTOP = "234880468147-mjls25oe8g86hjmmg9eoum9s028imlbi.apps.googleusercontent.com";
var OAUTH_CLIENT_SECRET_DESKTOP = "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";
...
var GoogleDriveSyncPlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.toggleAutoSync();
    this.registerObsidianProtocolHandler(MOBILE_REDIRECT_URI_SCHEME, async (params) => {
  ...
  // --- START: AUTHENTICATION (FINAL IMPLEMENTATION V3) ---
  toBase64Url(data) {
  ...
  async login() {
    if (this.isAuthenticating) {
      new import_obsidian.Notice("Authentication in progress.");
      return;
    }
    ...
    try {
      ...
      params.set("state", this.authState);
      params.set("code_challenge", challenge);
      params.set("code_challenge_method", "S256");
      params.set("access_type", "offline");
      params.set("prompt", "consent");
      if (import_obsidian.Platform.isDesktop) {
        params.set("client_id", OAUTH_CLIENT_ID_DESKTOP);
        params.set("redirect_uri", DESKTOP_REDIRECT_URI);
        ...
        params.set("client_id", OAUTH_CLIENT_ID_IOS);
        params.set("redirect_uri", `obsidian://${MOBILE_REDIRECT_URI_SCHEME}`);
        open(authUrl.toString());
      }
    } catch (err) {
      console.error("Login error:", err);
      new import_obsidian.Notice("Could not start login. See console.");
      stopLocalServer();
      this.isAuthenticating = false;
    }
  }
  async exchangeCodeForToken(code, codeVerifier) {
    ...
    try {
      ...
      if (import_obsidian.Platform.isDesktop) {
        payload.client_id = OAUTH_CLIENT_ID_DESKTOP;
        payload.client_secret = OAUTH_CLIENT_SECRET_DESKTOP;
        payload.redirect_uri = DESKTOP_REDIRECT_URI;
      } else {
        payload.client_id = OAUTH_CLIENT_ID_IOS;
        payload.redirect_uri = `obsidian://${MOBILE_REDIRECT_URI_SCHEME}`;
      }
      ...
      if (response.status !== 200) {
        console.error("Google API Error:", tokens);
        throw new Error(`Google API Error: ${tokens.error_description || tokens.error || "Unknown error"}`);
      }
      ...
      new import_obsidian.Notice("Failed to connect. See developer console.");
    } finally {
      this.isAuthenticating = false;
    }
  }
  ...
  // --- END: AUTHENTICATION (FINAL IMPLEMENTATION V3) ---
  toggleAutoSync() {
    if (this.autoSyncIntervalId) {
      window.clearInterval(this.autoSyncIntervalId);
      this.autoSyncIntervalId = null;
